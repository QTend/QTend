...
const Order = ({ cart, slug, table }:{cart :CartItem[], slug: any, table: string | null}) => {

  const totalPrice = cart.reduce((sum: any, item:any) => sum + item.price * item.quantity , 0);

  return (
    <Link href={`/${slug}/cart?table=${table}`} className="bg-[#4B2E05] py-3 px-5 rounded-2xl fixed bottom-5 right-6">
