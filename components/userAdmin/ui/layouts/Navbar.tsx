...
export function Navbar(){
    ...
    return(
        <nav className="flex max-w-7xl justify-between mx-auto mt-10 mb-5">
            ...
            {navs.map((n, index) => (
                <Link href={`/resturant/1/${n.label.toLowerCase()}`} key={index} className={`flex items-center gap-2 rounded-lg border border-black/10 px-3 py-1 cursor-pointer ${activeSegment === n.label.toLowerCase() ? 'text-white bg-[#68A544]' : 'text-black'}`}>
                    <div>{n.icon}</div>
                    <p>{n.label}</p>
                </Link>
            ))}
