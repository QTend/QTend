...
const faqs = [
    ...
    {id: 5, question: 'Does it require special hardware?Do customers need to download an app?', answer: "No special hardware needed. QTend works on any device with a browser — tablet, phone, or laptop. We provide printable QR code sheets."},
  ...
  return (
    <section className='p-16 bg-[#F7F7F7]'>
        ...
        <div className='max-w-[800px] mx-auto mt-16'>
            {faqs.map(f => (
                <div key={f.id} className='bg-white rounded-2xl p-4 mb-3 '>
                    <p className='font-bold text-base text-[#1D1D1F] mb-3'>{f.question}</p>
                    <p className='text-[#6B6B6B] text-sm'>{f.answer}</p>
                </div>
            ))}
        </div>
