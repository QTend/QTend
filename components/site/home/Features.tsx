...
export const Features = () => {
  return (
    <section className="p-16">
        ...
        <div className="grid grid-cols-5 gap-3 mt-16 max-w-desktop mx-auto">
            {features.map(f => {
                const IconComponent = f.icon

                return(
                    <div key={f.id} className="bg-[#F7F7F7] rounded-2xl p-5">
                        <div className="w-10 h-10 bg-[#FFFFFF] flex justify-center items-center border-[#E8E8E8] rounded-full mb-3">
                            <IconComponent color='#0E8A54' className=" w-5 h-5 " />
                        </div>
                        <p className="text-[#1D1D1F] font-bold text-sm mb-2">{f.label}</p>
                        <p className="text-[#6B6B6B] text-xs">{f.desc}</p>
